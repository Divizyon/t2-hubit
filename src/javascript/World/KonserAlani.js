...
export default class KonserAlani {
  ...
  KonserAlaniCollision2() {
    if (!this.physics) {
      console.warn('Konser alanı için physics parametresi verilmedi, collision eklenmeyecek.')
      return
    }
    
    // Kolon için sabit konum
    const columnPosition = new THREE.Vector3(-37.6, -24.3, 0)
    ...
    // Kolon boyutları - Y ekseni boyunca uzun bir kolon
    const columnWidth = 0.3  // X ekseni genişliği
    const columnHeight = 0.3 // Y ekseni yüksekliği (dikey uzunluk)
    const columnDepth = 10    // Z ekseni derinliği
